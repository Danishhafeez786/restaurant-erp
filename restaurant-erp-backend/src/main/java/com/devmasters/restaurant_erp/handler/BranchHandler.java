...
public class BranchHandler {
    ...
    public BranchModel create(BranchModel model) {
        ...
        if (branchService.existsByBranchNameIgnoreCase(model.getBranchName())) {
            throw new RuntimeException(
                    "Branch already exists with name : " + model.getBranchName()
            );
        }

        if (branchService.existsByBranchCodeIgnoreCase(model.getBranchCode())) {
            throw new RuntimeException(
                    "Branch already exists with code : " + model.getBranchCode()
            );
        }
    ...
    public PageResponse<BranchModel> getAll(
            BranchSearchCriteria criteria,
            Pageable pageable) {

        Page<Branch> page =
                branchService.search(criteria, pageable);

        return PageResponse.<BranchModel>builder()
                .content(
                        branchTransformer.toModels(page.getContent())
                )
    ...
    public BranchModel update(
            UUID id,
            BranchModel model) {

        Branch entity =
                branchTransformer.toEntity(model);

        Branch updated =
                branchService.update(id, entity);
